...
**Tag state: no tag exists.** Every gate of RELEASE.md passed on the release commit and private vulnerability reporting is now enabled, but the continuous-integration precondition was still unmet on GitHub's side when this record was written (table below). Contract 0.10.0 keeps 0.9.0's rule that no stable tag may be cut under that condition. When a run on this branch or on `main` executes with non-zero steps, RELEASE.md steps 3 to 6 apply to the merge commit of the release pull request without re-running the gates, provided no source has changed since `f3e13a0…`.
