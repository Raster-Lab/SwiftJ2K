...
**The current successor is the Milestone 2 migration baseline, not yet a replacement for J2KSwift.** `Encoder` and `Decoder` now encode, decode and inspect scalar lossless JPEG 2000 Part 1 codestreams for one unsigned greyscale component of 1–16 bits, one tile, one layer, reversible 5/3, raw codestream; `Transcoder` still advertises nothing. Every other predecessor feature an application relies on (HTJ2K, lossy and 9/7 coding, colour, tiles, JP2 containers, Metal acceleration, JPIP, JP3D, the daemon) is absent and rejected with `unsupportedFeature`. Keep the predecessor supplying those until the exact successor features your application needs are implemented and independently qualified. An unsupported error must not become an empty file, a black image or a successful migration result.
