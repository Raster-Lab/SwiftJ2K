...
**Release 12.1.0 task — 22 September 2026.** Assigned explicitly by the owner after the Milestone 5 merge `a964b7c34fef7ee58b3954ffaec813c1258376d1`, on branch `release/12.1.0`. Release commit `67273dbb5a195c1b2a1f47f311d9a1e4cd7f9203` changed only the version strings and the CHANGELOG; no codec, test or CLI logic changed. Every gate of `Documentation/RELEASE.md` was executed on that commit and recorded in `Documentation/RELEASE-12.1.0.md` with evidence under `Documentation/Engineering/Release-12.1.0/Evidence`. **No tag was created**: the continuous-integration precondition (every Actions job at `steps=0`) and the private-vulnerability-reporting precondition were both unmet, and contract 0.9.0 forbids a stable tag under either.
